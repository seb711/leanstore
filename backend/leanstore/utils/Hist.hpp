...
public:
   ...
   std::mutex lock;
    ...
    valueType step = (to - from) / static_cast<valueType>(size);
    std::cout << "[";
    for (int i = 0; i < size; ++i) {
        valueType interval_start = from + i * step;
        valueType interval_end = interval_start + step;
        std::cout << "(" << interval_start << "-" << interval_end << ")";
