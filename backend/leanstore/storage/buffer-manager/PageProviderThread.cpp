...
void BufferManager::handleWriteCompletion(mean::IoBaseRequest* request)
{
   ...
   while (true) {
      jumpmuTry()
      {
         ...
         assert(written_bf.header.lastWrittenGSN < written_gsn);

         if (FLAGS_out_of_place) {
            PID old_pid = written_bf.header.pid;
            PID new_pid = request->out_of_place_addr;
            written_bf.header.pid = new_pid;
            partition.freePage(old_pid);
         }

         written_bf.header.lastWrittenGSN = written_gsn;
